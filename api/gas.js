// Public gas endpoint (consolidated to stay within Vercel's function limit).
//   GET  /api/gas  → FahmanEnergy's current retail gas prices (edge-cached)
//   POST /api/gas  → create a refill reminder + send confirmation email
//
// POST body: { name?, email, cylinderKg?, dailyKg?, daysLeft?, runOut (YYYY-MM-DD), website? (honeypot) }

import { Resend } from 'resend';
import { insertGasAlert, getGasPrices } from './admin/_lib/db.js';
...
function parseDate(s) { if (!/^\d{4}-\d{2}-\d{2}$/.test(String(s || ''))) return null; const d = new Date(s + 'T12:00:00Z'); return isNaN(d.getTime()) ? null : d; }
function fmtDate(d) { return d.toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', timeZone: 'UTC' }); }

export default async function handler(req, res) {
  if (req.method === 'GET') return getPrices(res);
...
async function createAlert(req, res) {
  ...
  let stored = null;
  try {
    stored = await insertGasAlert({ name, email, cylinderKg, dailyKg, daysLeft, runOut: iso(runOut), remindOn: iso(remindOn), ip });
  } catch (err) {
    console.error('[gas] db insert failed', err);
  }
...
— FahmanEnergy, Kwara State, Nigeria`;
  ...
  try {
    ...
    if (error) {
      console.error('[gas] resend error', error);
      return ok(res, { id: stored?.id ?? null, emailDelayed: true, remindOn: iso(remindOn) });
    }
  } catch (err) {
    console.error('[gas] resend exception', err);
    return ok(res, { id: stored?.id ?? null, emailDelayed: true, remindOn: iso(remindOn) });
  }
